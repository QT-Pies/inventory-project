 ...
 */
Sale::Sale(const unsigned long id, const unsigned long sn, const std::string& d,
           const unsigned long as, const double sp, const double t,
 ...
 */
bool SaleList::addSale(const unsigned long id, const unsigned long sn,
                       const std::string& d, const unsigned long as,
                       const double sp, const double t, const std::string& b,
                       const std::string& s) {
...
bool SaleList::load() {
  ...
  if (line != "ID,Sale Number,Date,Amount Sold,Sale Price,Tax,Buyer,Seller")
    return false;

  while (!(fin.eof())) {
    std::getline(fin, line);
    sscanf(line.c_str(), "%lu,%lu,%s,%lu,%lf,%lf,%s,%s", &id, &sn, d, &as, &sp,
           &t, b, s);
    if (!addSale(id, sn, d, as, sp, t, b, s)) {
      std::cerr << "FILE CURRUPTION DETECTED in File: " << file_name
                << std::endl;
      return false;
    }
    offset++;
...
/*
 * Saves the sales information from the vector to the given file, uses append
 * and the offset value so only newly added information will be saved.
 */
bool SaleList::save() {
  std::ofstream fout;
  unsigned int i;
  ...
  for (i = offset; i < sales.size(); i++) {
    fout << std::endl
         << sales[i]->identification << "," << sales[i]->sale_number << ","
         << sales[i]->date << "," << sales[i]->amount_sold << ","
         << sales[i]->sale_price << "," << sales[i]->tax << ","
         << sales[i]->buyer << "," << sales[i]->seller;
  }
  fout.close();
  return true;
}
