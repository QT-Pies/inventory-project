  ...
  switch (argument) {
    ...
    case 'u':
      ...
      if (active_inventory->updateItem(name, category, value) != -1) {
        std::cout << "Updated " << category << " of " << name << " to " << value
                  << std::endl;
      }
      break;
    case 'Q':
...
void InventoryManager::readCSVFile() {
  ...
  while (csv_file.good()) {
    ...
    if (cat == "Perishable") {
      new_item = std::make_shared<PerishableItem>(
          name, cat, sub_cat, stoul(qty), stoul(str_id), stod(sale_price),
          stod(buy_cost), stod(tax), exp);
    } else if (cat == "NonPerishable") {
      new_item = std::make_shared<NonPerishableItem>(
          name, cat, sub_cat, stoul(qty), stoul(str_id), stod(sale_price),
          stod(buy_cost), stod(tax));
    } else {
      fprintf(stderr, "Invalid category.\n");
    }
...
int InventoryManager::fileOutput() {
  ...
  for (mit = active_inventory->inv_by_name.begin();
       mit != active_inventory->inv_by_name.end(); mit++) {
