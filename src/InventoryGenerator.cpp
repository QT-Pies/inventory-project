...
void InventoryGenerator::generateFile(const std::string& file_name, unsigned long unknown, unsigned long non_perishable, unsigned long perishable)
{
    unsigned long i;
...
std::shared_ptr<CSVEntry> InventoryGenerator::generateItem()
{
    ...
    else if (!random && isBadKey("quantity")) entry->quantity *= -1;

    entry->id = id_count;
    if (random && isBadKey("id") && distrib_keys(gen) != 0) id_count++;
    else if (!random && !isBadKey("id")) id_count++;

    entry->sale_price = distrib_double(gen);
    if (random && isBadKey("sale_price") && distrib_keys(gen) == 0) entry->sale_price *= -1;
    else if (!random && isBadKey("sale_price")) entry->sale_price *= -1;

    std::uniform_real_distribution<double> distrib_cost(0.05, entry->sale_price / 2);
    entry->buy_cost = distrib_cost(gen);
    if (random && isBadKey("buy_cost") && distrib_keys(gen) == 0) entry->buy_cost *= -1;
    else if (!random && isBadKey("buy_cost")) entry->buy_cost *= -1;

    entry->tax = 0.1;
    if (random && isBadKey("tax") && distrib_keys(gen) == 0) entry->tax = -0.1;
    else if (!random && isBadKey("tax")) entry->tax = -0.1;
