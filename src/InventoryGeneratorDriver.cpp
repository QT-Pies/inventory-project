...
int main(int argc, char** argv) {
  ...
  if (argc == 2 && (help_arg == "--help" || help_arg == "-h")) {
    ...
    std::cout << "\t\tWhen used with --bad, each specified bad key has 1 / bad_keys.size() chance of being a bad value"
              << std::endl;
    ...
    std::cout
        << "\t\tUsed with --random; larger numbers DECREASE the chance, while a value of 0 guarantees it happening."
        << std::endl;
