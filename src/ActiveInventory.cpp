...
int ActiveInventory::addItem(std::shared_ptr<Item> new_item) {
  if (inv_by_name.find(new_item->name) !=
      inv_by_name.end())  // Check if item is already in the inventory.
  {
...
int ActiveInventory::updateItem(std::string item_name, std::string field,
                                std::string value) {
  std::string cat;
  ...
  if (cat == "Perishable") {
    ...
    if (field == "name")  // If the name is to be changed we will have to
                          // re-enter it into the maps.
    {
      if (searchByName(value) != NULL) {
        ...
        inv_by_category[to_update->category].erase(
            inv_by_category[to_update->category].find(to_update->name));
